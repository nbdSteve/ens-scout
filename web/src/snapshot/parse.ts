...
/*
 * The primitive readers, bound to this parser's own failure. `src/wire/read.ts`
 * says why they are shared with the fresh-check parser and why `fail` is bound
 * rather than passed: a caller that branches on `SnapshotFormatError` to decide
 * whether to delete a cached snapshot has to keep being able to.
 */
const { asRecord, asArray, asString, asInteger, asCount, asInstant, asOptionalInstant, asLabel } =
  wireReaders(fail)
